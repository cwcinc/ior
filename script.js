...
function sendOut(response) {
  ...
  document.getElementById("p1").innerHTML = "<u>Name:</u> " + trackname + "<br><u>Plays:</u> " + plays + "<br><u>Likes:</u> " + likes + "<br><u>Dislikes:</u> " + dislikes + "<br><u>Verified:</u> " + verified.toString() + "<br><u>Creation Date:</u> " + created;
...
function wrCount(creatorLB) {
  document.getElementById("loading").innerHTML = "loading... please wait about 10 seconds";
  var fetches = [];
  for (let i = 0; i < 6; i++) {
    fetches.push(fetch("https://api.dashcraft.io/track/verified/" + i + "/50")
                 .then((response) => response.json())
                 .then((json) => {
      let IDarr = [];
      for (let a = 0; a < json.length; a++) {
        IDarr.push(json[a]._id);
      }
      return IDarr;
    }));
  }
  ...
  Promise.all(fetches)
    .then((IDL) => {
      ...
      IDtoPlayers(IDarr, creatorLB);
    });
}

function IDtoPlayers(IDs, creatorLB) {
  var fetches = [];
  var checkCheats = document.getElementById("cheatFilter").checked;
  var removeTop1 = document.getElementById("filterSpecial").checked;
  ...
  Promise.all(fetches)
    .then((users) => {
      if (creatorLB) {
        recorddict = condenseBasic(users);
      } else {
